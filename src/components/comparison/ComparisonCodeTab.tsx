...
export function ComparisonCodeTab({ compA, compB, codeTarget, setCodeTarget, codeTextA, codeTextB, lineDiffResult, onToast }: ComparisonCodeTabProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/90 p-3">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-100 dark:bg-zinc-800/80 p-0.5">
            <button
              ...
              className={`rounded-lg px-2.5 py-1 text-sm font-semibold transition-all cursor-pointer ${
                codeTarget === 'source'
          ...
          <div className="flex items-center rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-100 dark:bg-zinc-800/80 p-0.5">
            <button
              ...
              className={`inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1 text-sm font-semibold transition-all cursor-pointer ${
                diffViewMode === 'split'
                  ? 'bg-white dark:bg-zinc-700 text-zinc-900 dark:text-white'
          ...
          <label className="inline-flex items-center gap-1.5 text-sm text-zinc-600 dark:text-zinc-300 select-none cursor-pointer pl-2">
            <input
              type="checkbox"
        ...
        <div className="flex items-center gap-2">
          <span className="text-[13px] font-mono text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded-md font-semibold">
            +{lineDiffResult.stats.addedCount} añadidos
          </span>
          <span className="text-[13px] font-mono text-rose-600 dark:text-rose-400 bg-rose-500/10 px-2 py-0.5 rounded-md font-semibold">
            -{lineDiffResult.stats.removedCount} eliminados
          </span>
          <span className="text-[13px] font-mono text-zinc-600 dark:text-zinc-400 bg-zinc-100 dark:bg-zinc-800 px-2 py-0.5 rounded-md">
      ...
      {diffViewMode === 'split' ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-950 overflow-hidden">
            ...
            <div className="flex items-center justify-between border-b border-zinc-800 bg-zinc-900/80 px-4 py-2 text-sm">
              <div className="flex items-center gap-2">
                <span className="flex h-4 w-4 items-center justify-center rounded bg-indigo-600 text-white font-semibold text-sm">
                  A
                </span>
              ...
              <button
                ...
                className="inline-flex items-center gap-1 rounded-md px-2 py-0.5 text-[13px] text-zinc-600 dark:text-zinc-400 hover:text-white bg-zinc-800 hover:bg-zinc-700 transition-colors cursor-pointer"
              >
                {copiedA ? (
                  <>
            ...
            {/* Lines Left */}
            <div className="overflow-x-auto p-3 max-h-[580px] text-sm font-mono leading-relaxed select-text">
                  ...
                  {lineDiffResult.sideBySide
                    ...
                    .map((pair, idx) => {
                      ...
                      return (
                        ...
                        >
                          <td className="w-10 select-none pr-3 text-right text-zinc-600 text-sm align-top">
                            {line.lineNum ?? ''}
                          </td>
                          <td className="w-4 select-none text-center font-semibold text-sm text-rose-400">
                            {isRemoved ? '-' : ''}
                          </td>
          ...
          <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-950 overflow-hidden">
            ...
            <div className="flex items-center justify-between border-b border-zinc-800 bg-zinc-900/80 px-4 py-2 text-sm">
              <div className="flex items-center gap-2">
                <span className="flex h-4 w-4 items-center justify-center rounded bg-emerald-600 text-white font-semibold text-sm">
                  B
                </span>
                <span className="font-mono text-zinc-200 font-semibold">
                  {compB?.name}.tsx
                    ...
                    .map((pair, idx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="w-4 select-none text-center font-semibold text-sm text-emerald-400">
                            {isAdded ? '+' : ''}
                          </td>
                          <td className="whitespace-pre overflow-x-hidden">
                            {line.text || (isEmpty ? ' ' : '')}
        ...
        <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-950 overflow-hidden">
          ...
          <div className="overflow-x-auto p-4 max-h-[600px] text-sm font-mono leading-relaxed select-text">
            <table className="border-collapse w-full">
              <tbody>
                    ...
                    <tr
                      ...
                      className={`${
                        line.type === 'added'
                          ...
                          : line.type === 'removed'
                          ? 'bg-rose-950/40 text-rose-200 border-l-2 border-rose-500'
                          : 'text-zinc-600 dark:text-zinc-400 hover:bg-zinc-900/30'
                      }`}
                    >
                      <td className="w-10 select-none pr-2 text-right text-zinc-600 text-sm">
                        {line.lineNumA ?? ''}
                      </td>
                      <td className="w-10 select-none pr-2 text-right text-zinc-600 text-sm">
                        {line.lineNumB ?? ''}
                      </td>
                      <td className="w-4 select-none text-center font-semibold text-[13px]">
                        {line.type === 'added'
                          ? '+'
                          : line.type === 'removed'
                          ? '-'
