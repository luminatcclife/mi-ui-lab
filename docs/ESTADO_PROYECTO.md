...
2. **Arreglar la capa de datos**: quitar localStorage de `App.tsx` (Dexie como única fuente de verdad), corregir el import que borra piezas del estado, el favorito fantasma `custom-button` y la desincronización de favoritos al borrar; extraer un hook `useCatalog`.
3. **Poner una red de seguridad**: Vitest más tests de las 6 utilidades puras y de `db.ts`; añadir `npm test` a CI; activar `strict` en TypeScript; error boundary global y por pieza.
